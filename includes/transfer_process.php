...
if (isset($_POST['port_transfer_submit']) && !empty($_POST['port_transfer_submit'])) {
	
	// check duplicate:
	$port_transfer_id		= $_POST['port_transfer_id'];
    $table		= 'inv_port_transfer';
    $where		= "port_transfer_id='$port_transfer_id'";
    if(isset($_POST['port_transfer_update_submit']) && !empty($_POST['port_transfer_update_submit'])){
        $notWhere   =   "id!=".$_POST['port_transfer_update_submit'];
        $duplicatedata = isDuplicateData($table, $where, $notWhere);
    }else{
        $duplicatedata = isDuplicateData($table, $where);
    }
    ...
    for ($count = 0; $count < count($_POST['quantity']); $count++) {
         ...
         *  Insert Data Into inv_transferdetail Table:
        */ 
        $port_transfer_date		= $_POST['port_transfer_date'];
        $port_transfer_id		= $_POST['port_transfer_id'];
        $type					= 'p2p';
        ...
        $query = "INSERT INTO `inv_port_transferdetails` (`port_transfer_id`,`material_id`,`material_name`,`transfer_qty`,`unit`,`type`,`in_port`,`in_warehouse`,`out_port`,`out_warehouse`) VALUES ('$port_transfer_id','$material_id','$material_name','$quantity','$unit','1','$to_port','$to_warehouse','$from_port','$from_warehouse')";
        $conn->query($query);
        
        /*
         *  Insert Data Into inv_materialbalance Table:
        */
        $mb_ref_id      = $port_transfer_id;
        $mb_materialid  = $material_id;
        $mb_date        = (isset($port_transfer_date) && !empty($port_transfer_date) ? date('Y-m-d h:i:s', strtotime($port_transfer_date)) : date('Y-m-d h:i:s'));
        $mbfrom_in_qty       = 0;
        $mbfrom_out_qty      = $quantity;
        $mbfrom_type         = 'Transfer Out';
		...
		$mbin_in_qty       = $quantity;
        $mbin_out_qty      = 0;
        $mbin_type         = 'Transfer In';
		
        $mbserial       = '1.1';
        $mbunit_id      = $unit;
        $mbserial_id    = 0;
        $jvno           = $port_transfer_id;       
        
        $query_outmb = "INSERT INTO `inv_materialbalance` (`mb_ref_id`,`mb_materialid`,`mb_date`,`mbin_qty`,`mbin_val`,`mbout_qty`,`mbout_val`,`mbprice`,`mbtype`,`mbserial`,`mbserial_id`,`mbunit_id`,`jvno`, `port_id`, `warehouse_id`) VALUES ('$mb_ref_id','$mb_materialid','$mb_date','$mbfrom_in_qty','0','$mbfrom_out_qty','0','0','$mbfrom_type','$mbserial','$mbunit_id','$mbserial_id','$jvno','$from_port','$from_warehouse')";
        $conn->query($query_outmb);
		
		 $query_inmb = "INSERT INTO `inv_materialbalance` (`mb_ref_id`,`mb_materialid`,`mb_date`,`mbin_qty`,`mbin_val`,`mbout_qty`,`mbout_val`,`mbprice`,`mbtype`,`mbserial`,`mbserial_id`,`mbunit_id`,`jvno`, `port_id`, `warehouse_id`) VALUES ('$mb_ref_id','$mb_materialid','$mb_date','$mbin_in_qty','0','$mbin_out_qty','0','0','$mbin_type','$mbserial','$mbunit_id','$mbserial_id','$jvno','$to_port','$to_warehouse')";
        $conn->query($query_inmb);


    }
    /*
    *  Insert Data Into inv_port_transfer Table:
    */
    $query2 = "INSERT INTO `inv_port_transfer` (`port_transfer_id`,`port_transfer_date`,`type`,`from_port`,`from_warehouse`,`to_port`,`to_warehouse`,`remarks`) VALUES ('$port_transfer_id','$port_transfer_date','$type','$from_port','$from_warehouse','$to_port','$to_warehouse','$remarks')";
    $result2 = $conn->query($query2);    
  
    
    $_SESSION['success']    =   "P2P transfer process have been successfully completed.";
    header("location: port_transfer.php");
    exit();
	}

}

/*========================Port To Site Transfer==================================================*/

if (isset($_POST['port_to_site_transfer_submit']) && !empty($_POST['port_to_site_transfer_submit'])) {
    ...
    */
    ...
    $_SESSION['success']    =   "P2P transfer process have been successfully completed.";
    header("location: port_to_site_transfer.php");
    exit();
	}
...
if (isset($_POST['site_to_site_transfer_submit']) && !empty($_POST['site_to_site_transfer_submit'])) {
    ...
    for ($count = 0; $count < count($_POST['quantity']); $count++) {
         ...
         *  Insert Data Into inv_transferdetail Table:
        */ 
        $port_transfer_date		= $_POST['site_transfer_date'];
        $port_transfer_id		= $_POST['site_transfer_id'];
        $type					= 's2s';
    ...
    */
    ...
    header("location: site_to_site_transfer.php");
    exit();
	}
