 ...
 * *****************************************************************************
 */
if (isset($_POST['project_return_submit']) && !empty($_POST['project_return_submit'])) {
    $receive_total      =   0;
    $no_of_material     =   0;
    for ($count = 0; $count < count($_POST['quantity']); $count++) {
        ...
        */       
        ...
        $query = "INSERT INTO `inv_project_returndetail` (`return_id`,`return_date`,`material_id`,`material_name`,`unit`,`return_qty`,`return_price`,`part_no`,`inproject`,`outproject`,`inwarehouse`,`outwarehouse`) VALUES ('$return_id','$return_date','$material_id','$material_name','$unit','$quantity','$unit_price','$part_no','$to_project','$from_project','$to_warehouse','$from_warehouse')";
        $conn->query($query);
        ...
        */
        ...
        $mbout_qty      = 0;
        $mbout_val      = 0;
        $mbtype         = 'ProjectReturn';		
        
        $query_inmb = "INSERT INTO `inv_materialbalance` (`mb_ref_id`,`mb_materialid`,`mb_date`,`mbin_qty`,`mbin_val`,`mbout_qty`,`mbout_val`,`mbprice`,`mbtype`,`mbserial`,`mbserial_id`,`mbunit_id`,`jvno`,`part_no`,`project_id`,`warehouse_id`) VALUES ('$mb_ref_id','$mb_materialid','$mb_date','$mbin_qty','$mbin_val','$mbout_qty','$mbout_val','$mbprice','$mbtype','$mbserial','$mbunit_id','$mbserial_id','$jvno','$part_no','$to_project','$to_warehouse')";
    ...
    */
    ...
    $query2 		= "INSERT INTO `inv_project_return` (`return_id`,`return_date`,`remarks`,`from_project`,`to_project`,`from_warehouse`,`to_warehouse`) VALUES ('$return_id','$return_date','$remarks','$from_project','$to_project','$from_warehouse','$to_warehouse')";
    $result2 = $conn->query($query2);
    
    $_SESSION['success']    =   "return process have been successfully completed.";
    header("location: project_return_entry.php");
    exit();
}
