			...
			<div class="card-body" id="printableArea">
				<div class="row">
					...
					<div class="col-sm-10">
							...
							<div class="col-sm-6">
								<table class="table table-bordered">
									...
									<tr>
										<th>Transfer Date:</th>
										<td><?php
											$sqld = "select * from `inv_port_transfer` where `port_transfer_id`='$transfer_id'";
											$resultd = mysqli_query($conn, $sqld);
											$rowd = mysqli_fetch_array($resultd);
										echo $rowd['port_transfer_date'] ?></td>
									</tr>
									<tr>
										<th>From Port:</th>
										<td>
											<?php 
											$dataresult =   getDataRowByTableAndId('ports', $rowd['from_port']);
						...
						<center><button class="btn btn-block btn-secondary challan">PROJECT TO PROJECT TRANSFER DETAILS</button></center>
						<table class="table table-bordered" id="material_receive_list"> 
							...
							<tbody id="material_receive_list_body">
								<?php
								$transfer_id=$_GET['no'];
								$sql = "select * from `inv_port_transferdetails` where `port_transfer_id`='$transfer_id'";
								...
								<?php } ?>
								<tr>
									<td colspan="3" class="grand_total" style="text-align:right;">
										<b>Grand Total:</b>
									</td>
									<td>
										<?php 
										$sql2 = "SELECT sum(transfer_qty) FROM  `inv_port_transferdetails` where `port_transfer_id`='$transfer_id'";
